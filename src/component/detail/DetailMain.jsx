...
import { getDetailProductApi } from "../../api/posts";
...
function DetailMain() {
  ...
  // 게시물 상세조회
  const { data, isLoading, isError } = useQuery(["posts", id], async () => {
        ...
        <MainWrapper>
          <MainContainer>
            ...
            {data.imageUrl.map((item, key) => {
              return (
                <MainArticle key={key}>
                  <ArticleImage>
                    <ArticleImageSrc
                      src={`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTwbqz0RKvg6rUoehH3z81Fm3LP4fI-gHuspA&usqp=CAU`}
                      alt=""
                    />
                  </ArticleImage>
                </MainArticle>
            ...
            <ArticleContent>
              ...
              <div>재고 수 : {data.stockCount} 개</div>
            </ArticleContent>
            <CountSection>
              <CountDl>
                <CountDt>조회</CountDt>
